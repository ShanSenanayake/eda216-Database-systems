...
public class BookingPane extends BasicPane {
	 ...
	 * @param dates
	 */
	private void fillDateList(String[] dates) {
	...
	class NameSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			String[] dates = db.fetchDates(movieName);
			fillDateList(dates);
	...
	class DateSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			String[] theatreInfo = db.getTheatreInfo(movieName, date);
			fields[2].setText(theatreInfo[0]);
			fields[3].setText(theatreInfo[1]);
		}
	}
	...
	class ActionHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			String movieName = nameList.getSelectedValue();
			String date = dateList.getSelectedValue();
			int reservNbr = db.bookSeat(movieName, date, CurrentUser.instance()
					.getCurrentUserId());
			if (reservNbr < 0) {
				displayMessage("Booking unsuccessfull, no seats available");
			} else {
				displayMessage("One ticket booked, Booking number: "
						+ reservNbr);
				int seatNbr = Integer.parseInt(fields[3].getText())-1;
				fields[3].setText(Integer.toString(seatNbr));
			}
		}
	}
}
